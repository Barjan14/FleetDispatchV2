...
export default function AdminDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      setUsers((uRes.data || []).map(usr => ({
        ...
        profile: { employee_id: usr.employee_id || '', department: usr.department || '', phone: usr.phone || '' },
      })));
  ...
  const fetchLogsSilently = useCallback(async () => {
    try {
      const [vehicleRes, tripRes] = await Promise.all([
        ...
        supabase.from('trip_logs').select('*, vehicles(id, name)').order('created_at', { ascending: false })
      ]);
  ...
  const bookingAction = async (id, status, vehicleId = null, driverId = null) => {
    ...
    if (status === 'Approved') {
      ...
      await supabase.from('driver_profiles').update({ availability: 'On Trip', assigned_vehicle_id: vehicleId }).eq('id', driverId);
    }
    ...
    fetchAll(); // Refresh manually after action
  };
  ...
  return (
    <div className="admin-root">
      ...
      <main className="admin-main">
        ...
        {tab === 'bookings' && (
          <BookingsPage 
            ...
            onRefresh={fetchAll} 
            /* ✅ Tell the button how to open the modal */
        ...
        {tab === 'fleets' && (
          <FleetsPage
            ...
            onRefresh={fetchAll} /* ✅ Pass refresh down */
          />
        )}
      ...
      {modalType === 'vehicleDetails' && (
        <VehicleDetailsModal vehicle={selectedVehicle} onEdit={() => openEditVehicle(selectedVehicle)} onClose={() => setModalType('')} />
      )}
      {(modalType === 'addVehicle' || modalType === 'editVehicle') && (
        <VehicleFormModal mode={modalType === 'addVehicle' ? 'add' : 'edit'} data={vForm} onChange={setVForm} onSave={saveVehicle} onClose={() => setModalType('')} />
      )}
      {(modalType === 'addUser' || modalType === 'editUser') && (
        <UserFormModal mode={modalType === 'addUser' ? 'add' : 'edit'} data={uForm} onChange={setUForm} onSave={saveUser} onClose={() => setModalType('')} />
      )}
    </div>
  );
}