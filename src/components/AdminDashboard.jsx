...
export default function AdminDashboard() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      setOngoingBookings((bRes.data || []).filter(b => b.status === 'Ongoing'));
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(err => { console.error(err); showToast('Failed to load logs', 'err'); })
  ...
  const bookingAction = async (id, status, vehicleId = null, driverId = null) => {
    ...
    try {
      ...
      if (status === 'Approved') {
        ...
        if (vId) await supabase.from('vehicles')
          .update({ is_available: false }).eq('id', vId);
        if (dId) await supabase.from('driver_profiles')
          .update({ availability: 'On Trip', assigned_vehicle_id: vId || null }).eq('id', dId);
      }

      if (status === 'Ongoing') {
        const vId = booking.vehicle_id;
        const dId = booking.driver_id;
        if (vId) await supabase.from('vehicles')
          .update({ is_available: false }).eq('id', vId);
        if (dId) await supabase.from('driver_profiles')
          .update({ availability: 'On Trip', assigned_vehicle_id: vId || null }).eq('id', dId);
      ...
      // 3. Send approval email (only when Approved)
      if (status === 'Approved') {
        ...
        console.log('Triggering approval email for:', booking.email || booking.user?.email);
        ...
        if (funcError) {
          ...
          // Show warning but don't block — booking is already approved
