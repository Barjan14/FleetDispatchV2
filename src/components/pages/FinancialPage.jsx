...
const css = `
  .fin-root { display: flex; flex-direction: column; gap: 24px; }
  .fin-tabs { display: flex; gap: 8px; flex-wrap: wrap; }
  .fin-tab {
    padding: 8px 18px; border-radius: 8px; border: 1.5px solid ${C.border};
    background: ${C.card}; color: ${C.muted}; font-size: 13px; font-weight: 600;
  ...
  .fin-summary { display: grid; grid-template-columns: repeat(4,1fr); gap: 14px; }
  .fin-stat {
    background: ${C.card}; border-radius: 12px; padding: 18px 20px;
    box-shadow: 0 1px 4px rgba(0,0,0,.06); display: flex; align-items: center; gap: 14px;
  }
  .fin-stat-icon { width: 44px; height: 44px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 20px; }
  .fin-stat-info p { font-size: 11px; color: ${C.muted}; font-weight: 600; text-transform: uppercase; letter-spacing: .5px; }
  .fin-stat-info h3 { font-size: 22px; font-weight: 700; margin-top: 2px; }

  .fin-card { background: ${C.card}; border-radius: 12px; box-shadow: 0 1px 4px rgba(0,0,0,.06); overflow: hidden; }
  .fin-card-header {
    display: flex; align-items: center; justify-content: space-between;
    padding: 16px 22px; border-bottom: 1px solid ${C.border};
  }
  .fin-card-header h3 { font-size: 15px; font-weight: 600; }
  .fin-table { width: 100%; border-collapse: collapse; }
  .fin-table thead tr { background: #f8fafc; }
  .fin-table th { padding: 10px 18px; text-align: left; font-size: 11px; font-weight: 700; color: ${C.muted}; text-transform: uppercase; letter-spacing: .5px; }
  .fin-table td { padding: 12px 18px; font-size: 13px; border-top: 1px solid ${C.border}; vertical-align: middle; }
  .fin-table tr:hover td { background: #f8fafc; }
  .fin-empty { text-align: center; color: ${C.muted}; padding: 32px; font-size: 14px; }

  .fin-btn { padding: 7px 14px; border: none; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 500; transition: opacity .15s; }
  ...
  .fin-fg input, .fin-fg select, .fin-fg textarea {
    padding: 8px 11px; border: 1px solid ${C.border}; border-radius: 7px;
    ...
    <>
      ...
      <div className="fin-card">
        <div className="fin-card-header">
          ...
          <button className="fin-btn fin-btn-primary" onClick={() => { setForm({ vehicle_id:'',date:'',liters:'',cost_per_liter:'',total_cost:'',odometer_km:'',station_name:'',notes:'' }); setModal(true); }}>+ Add Fuel Record</button>
        </div>
...
function MaintenanceTab({ vehicles }) {
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal]     = useState(false);
  const [toast, setToast]     = useState('');
              ...
              <div className="fin-row2">
                ...
                <div className="fin-fg">
                  ...
                  <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}>
                    {['Fuel','Repair','Parts','Insurance','Registration','Depreciation','Other'].map(c => (
...
function RepairsTab({ vehicles }) {
  ...
  const updateStatus = async (id, status) => {
    await supabase.from('repair_records').update({
      status,
      date_resolved: status === 'Completed' ? new Date().toISOString().split('T')[0] : null,
    }).eq('id', id);
    load();
  };
...
// ── Depreciation Tab ──────────────────────────────────────
function DepreciationTab({ vehicles }) {
  const [records, setRecords] = useState([]);
  const [loading, setLoading] = useState(true);
  const [modal, setModal]     = useState(false);
  const [toast, setToast]     = useState('');
  ...
  const load = useCallback(async () => {
    const { data } = await supabase
      ...
      .select('*, vehicles(name, plate_number, year)')
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('vehicle_depreciation').upsert({
      ...
      useful_life_years:   uly,
      residual_value:      rv,
      current_value:       cv ? parseFloat(cv.toFixed(2)) : null,
    ...
    <div className="fin-root">
      ...
      <div className="fin-tabs">
        {TABS.map(t => (
          <button
            key={t.key}
            className={`fin-tab ${activeTab === t.key ? 'active' : ''}`}
      ...
      {activeTab === 'maintenance'  && <MaintenanceTab  vehicles={vehicles} />}
      {activeTab === 'repairs'      && <RepairsTab      vehicles={vehicles} />}
      {activeTab === 'depreciation' && <DepreciationTab vehicles={vehicles} />}
    </div>
  );
}